...
// Layout giống user-score.tsx, có thêm nút XEM RANK CỦA BẠN
...
// ── Assets ────────────────────────────────────────────────────────────────────
const CLEAN_1 = require('../../assets/images/clean_1.png');
const CLEAN_2 = require('../../assets/images/clean_2.png');
const CLEAN_3 = require('../../assets/images/clean_3.png');
const CLEAN_4 = require('../../assets/images/clean_4.png');
const CLEAN_5 = require('../../assets/images/clean_5.png');
...
function GradientBar({ pct, colors }: { pct: number; colors: [string, string, ...string[]] }) {
  return (
...
function CatRow({ icon, label, score }: { icon: any; label: string; score: number | undefined }) {
...
const row = StyleSheet.create({
  ...
  label:    { fontFamily: FONTS.MONO, fontSize: 13, color: COLORS.TEXT_PRIMARY, letterSpacing: 0.5, width: 72 },
  barTrack: { flex: 1, height: 10, backgroundColor: 'rgba(255,255,255,0.10)', borderRadius: 5, overflow: 'hidden' },
  barFill:  { height: '100%', borderRadius: 5 },
  score:    { fontFamily: FONTS.MONO_BOLD, fontSize: 14, color: COLORS.TEXT_PRIMARY, width: 28, textAlign: 'right', letterSpacing: 0.5 },
});
...
export default function MyScoreScreen() {
  ...
  try { scoreData = JSON.parse(dataStr ?? ''); } catch { /* invalid */ }

  const handleBack     = () => router.back();
  const handleViewRank = () => router.push('/(premium)/leaderboard');
    ...
    return (
      <BxhBackground>
        <View style={s.errWrap}>
          <Text style={s.errText}>Không tải được dữ liệu.</Text>
  ...
  const tierColor      = getTierColor((scoreData.pslTier ?? 'HTN') as PSLTier);
  const potentialColor = getTierColor((scoreData.potentialTier ?? scoreData.pslTier ?? 'HTN') as PSLTier);
  const overallScore   = Math.round(scoreData.combinedScore);
  const potentialScore = tierToScore(scoreData.potentialTier ?? scoreData.pslTier ?? '');
  const overallPct     = overallScore / 100;
  const potentialPct   = potentialScore / 99;
  const initial        = (scoreData.username ?? 'Y').charAt(0).toUpperCase();

  const catScores: Record<string, number | undefined> = {
    appeal: scoreData.appealScore,
    jaw:    scoreData.jawScore,
    eyes:   scoreData.eyesScore,
    hair:   scoreData.hairScore,
    nose:   scoreData.noseScore,
  };
        ...
        <View style={s.screen}>
          ...
          <View style={s.header}>
            ...
            <Text style={s.username} numberOfLines={1}>
              {(scoreData.username ?? 'YOU').toUpperCase()}
            </Text>
            <View style={{ width: 32 }} />
          </View>
            ...
            <View style={[s.avatarOuterGlow, { shadowColor: tierColor }]}>
              <View style={[s.avatarInnerGlow, { shadowColor: tierColor }]}>
                <View style={[s.avatarRing, { borderColor: tierColor }]}>
                  {scoreData.photoUri ? (
                    <Image source={{ uri: scoreData.photoUri }} style={s.avatarImg} resizeMode="cover" />
                  ) : (
          ...
          <View style={s.bottomBlock}>
            ...
            <TouchableOpacity style={s.rankBtn} onPress={handleViewRank} activeOpacity={0.75}>
              <Text style={s.rankBtnText}>XEM RANK CỦA BẠN →</Text>
            </TouchableOpacity>
